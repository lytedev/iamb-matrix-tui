    ...
    fn changenum(
        ...
        while cursor.y <= range.end.y {
            ...
            let number = match self.text.get_cursor_word_mut(&mut cursor, &style) {
                Some(n) => n,
                // A failed seek means that no number follows the cursor anywhere in the
                // buffer. Stop here. A `continue` would repeat the same failed seek forever.
                None => break,
            };
            ...
            let n = match n.as_ref().parse::<isize>() {
                ...
                // The digits do not fit in an isize. Move past them and look for the next
                // number. A bare `continue` would find the same digits forever.
                Err(_) => {
                    let here = usize::from(self.text.cursor_to_offset(&cursor));
                    let last = usize::from(self.text.last_offset());
                    let past = here.saturating_add(nlen);

                    if past > last {
                        break;
                    }

                    cursor = self.text.offset_to_cursor(past.into());
                    continue;
                },
            };
...
mod tests {
    ...
    #[test]
    fn test_changenum_no_number_terminates() {
        // A line with no number must not make changenum loop forever.
    ...
    #[test]
    fn test_changenum_unparsable_terminates() {
        // A digit string too large for isize must not make changenum loop forever.
        let big = "9".repeat(64);
        let text = format!("a {big} b\n");
        let (mut ebuf, curid, vwctx, mut vctx, mut store) = mkfivestr(text.as_str());
