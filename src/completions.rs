...
use crate::message::emoji::{EMOJI_SIGIL, complete_emoji_names, complete_emojis};
use crate::message::mention::{MENTION_SIGIL, MentionCandidate, complete_mentions};
...
fn complete_emoji(input: &str, _store: &ChatStore) -> Vec<String> {
    complete_emoji_names(input)
}
...
fn complete_iamb_unreads(args: Vec<String>) -> Vec<String> {
    match args.len() {
        1 => complete_choices(&args[0], &["clear", "threads"]),
        _ => vec![],
    }
}
...
fn complete_cmdarg(
    ...
    *cursor = new_cursor;
    ...
    let mut completions = match cmd.name.as_str() {
        ...
        "read" if args.len() == 1 => complete_choices(&args[0], &["all", "here"]),
        "read" => vec![],

        "reindex" if args.len() == 1 => complete_choices(&args[0], &["all", "status", "stop"]),
        "reindex" => vec![],

        // A snooze duration, a search term, and a shell command are all free text.
...
fn complete_msgbar(
    ...
    match id.chars().next() {
        ...
        Some('#') => store.names.complete(id.as_ref()),
        ...
        Some(EMOJI_SIGIL) => complete_emojis(id.as_ref()),

        // Complete a mention of somebody in this room.
        Some(MENTION_SIGIL) => complete_mention(id.as_ref(), room_id, store),
