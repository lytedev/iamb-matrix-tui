...
impl RoomState {
    ...
    /// Run the room actions that need this window's own state, such as its selected message or
    /// the thread it shows. Every other room action goes to [room_command], which only needs the
    /// room id and therefore also serves the list windows.
    pub async fn room_command(
        &mut self,
        act: RoomAction,
        ctx: ProgramContext,
        ...
        match act {
            ...
            RoomAction::Snooze(when) => {
                ...
                store
                    .application
                    .snooze
                    .set(SnoozeKey { room_id: room_id.clone(), thread }, wake_at);
            ...
            act => room_command(self.id(), act, ctx, store).await,
        }
    }
