...
impl ChatState {
    ...
    pub fn selected_event(&self, info: &mut RoomInfo) -> Option<OwnedEventId> {
        self.scrollback
            .get_key(info)
            .and_then(|key| key.id.as_origin().map(ToOwned::to_owned))
    }
    ...
    pub fn accept_completion(&mut self, ctx: &ProgramContext, store: &mut ProgramStore) -> bool {
        ...
        let take = EditorAction::Complete(
            CompletionStyle::List(MoveDir1D::Next, false),
            CompletionType::Auto,
            CompletionDisplay::List,
        );
    ...
    pub async fn send_command(
        ...
        match act {
            ...
            SendAction::UploadImage(width, height, bytes, add_caption) => {
                let image = ClipboardImage { width, height, bytes };

                self.send_clipboard_image(&room, image, add_caption, info, settings)
                    .await?;
            },
            SendAction::UploadClipboard => {
                let image = clipboard_image(&store.registers)?;

                // Pasting is one action, so the message bar is taken as the caption without
                // asking: it is right there on screen, and an empty bar sends the image alone.
                self.send_clipboard_image(&room, image, true, info, settings).await?;
            },
        }
        ...
        if tunables.read_receipt_trigger.on_message() &&
            let Some(replies) = self.scrollback.get_thread(info).and_then(|t| t.replies())
        {
    ...
    async fn send_clipboard_image(
        ...
        room.send_queue()
            .send_attachment(name.as_str(), mime, bytes, config)
            .await
            .map_err(IambError::from)?;
...
mod tests {
    ...
    fn test_highlight_matches_what_the_first_keypress_takes() {
        ...
        let mut list = completion_list(None);
        let taken = list.select(&CompletionStyle::List(MoveDir1D::Next, false));
